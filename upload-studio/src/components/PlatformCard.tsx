...
export function PlatformCard({ platform, onClick }: PlatformCardProps) {
  return (
    <button
      className="panel group relative flex min-h-[92px] items-center justify-between overflow-hidden px-5 text-left transition hover:-translate-y-0.5 hover:border-cyan-300/60 hover:shadow-glow"
      onClick={() => onClick(platform)}
      type="button"
      title={platform.connected ? `Reconnect ${platform.label}` : `Authenticate ${platform.label}`}
    >
      ...
      <div className="flex items-center gap-4">
        ...
        <ChevronRight className="text-studio-muted transition group-hover:text-studio-text" size={20} />
      </div>
    </button>
  );
}
